	...
	: Super(ObjectInitializer)
{
	...
	SpineLeftAttachPoint= TEXT("SpineLeftSocket");
	PelvisLeftAttachPoint = TEXT("PelvisLeftSocket");
	NeckAttachPoint = TEXT("NeckSocket");
}
...
void ASCharacter::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	...
	InputComponent->BindAction("EquipTertiaryWeapon", IE_Pressed, this, &ASCharacter::OnEquipTertiaryWeapon);
	InputComponent->BindAction("EquipUtilityWeapon", IE_Pressed, this, &ASCharacter::OnEquipUtilityWeapon);
...
void ASCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	/* DOREPLIFETIME_CONDITION(ASCharacter, WeaponInventory, COND_OwnerOnly);*/
}
...
FName ASCharacter::GetInventoryAttachPoint(EInventorySlot Slot) const
{
	...
	case EInventorySlot::Tertiary:
		return SpineLeftAttachPoint;
	case EInventorySlot::Utility:
		return PelvisLeftAttachPoint;
	default:
		// Not implemented.
		return "";
	}
...
void ASCharacter::DestroyInventory()
{
	...
	{
		ASWeapon* Weapon = WeaponInventory[i];
		if (Weapon)
		{
...
void ASCharacter::AddWeapon(class ASWeapon* Weapon)
{
	if (Weapon && Role == ROLE_Authority)
	{
		...
		if (WeaponInventory.Num() > 0 && CurrentWeapon == nullptr)
		{
...
void ASCharacter::RemoveWeapon(class ASWeapon* Weapon, bool bDestroy)
{
	if (Weapon && Role == ROLE_Authority)
	{
		...
		if (bIsCurrent && WeaponInventory.Num() > 0)
		{
...
void ASCharacter::OnNextWeapon()
{
	...
	if (WeaponInventory.Num() >= 2) // TODO: Check for weaponstate.
	{
		const int32 CurrentWeaponIndex = WeaponInventory.IndexOfByKey(CurrentWeapon);
		ASWeapon* NextWeapon = WeaponInventory[(CurrentWeaponIndex + 1) % WeaponInventory.Num()];
...
void ASCharacter::OnPrevWeapon()
{
	...
	{
		const int32 CurrentWeaponIndex = WeaponInventory.IndexOfByKey(CurrentWeapon);
		ASWeapon* PrevWeapon = WeaponInventory[(CurrentWeaponIndex - 1 + WeaponInventory.Num()) % WeaponInventory.Num()];
...
void ASCharacter::OnEquipTertiaryWeapon()
{
	if (CarriedObjectComp->GetIsCarryingActor())
	{
		CarriedObjectComp->Rotate(-1.0f, 0.0f);
...
//	if (WeaponInventory.Num() >= 2)
//	{
		/* Find first weapon that uses tertiary slot. */
		for (int32 i = 0; i < WeaponInventory.Num(); i++)
		{
			ASWeapon* Weapon = WeaponInventory[i];
			if (Weapon->GetStorageSlot() == EInventorySlot::Tertiary)
			{
				EquipWeapon(Weapon);
			}
		}
...
void ASCharacter::OnEquipUtilityWeapon()
{
	if (CarriedObjectComp->GetIsCarryingActor())
	{
		CarriedObjectComp->Rotate(-1.0f, 0.0f);
	...
	//	if (WeaponInventory.Num() >= 2)
	//	{
	/* Find first weapon that uses utility slot. */
	for (int32 i = 0; i < WeaponInventory.Num(); i++)
	{
		ASWeapon* Weapon = WeaponInventory[i];
		if (Weapon->GetStorageSlot() == EInventorySlot::Utility)
		{
			EquipWeapon(Weapon);
		}
	}
...
bool ASCharacter::WeaponSlotAvailable(EInventorySlot CheckSlot)
{
	...
	//return nullptr == WeaponInventory.FindByPredicate([=](ASWeapon* W){ return W->GetStorageSlot() == CheckSlot; });
}
